...
const burger = (menuSelector,burgerSelector) => {
	const menuElem = document.querySelector(menuSelector),
		...
		//availWidth - ширина браузре без лишниъ элементов
...
const filter = () => {
	...
	const typeFilter = (markType) => {
		//Скрываем все элементы потому что есть класс all
		markAll.forEach(mark => {
			mark.style.display = 'none';
			mark.classList.remove('animated','fadeIn');
		});
		//скрываем поле "нет таких элементов"
		no.style.display = 'none';
		no.classList.remove('animated','fadeIn');
		//если передали фильтр - отображаем все элементы с этим классом
		if (markType) {
			markType.forEach(mark => {
				mark.style.display = 'block';
				mark.classList.add('animated','fadeIn');
			});		//иначе выводим поле "нет таких элементов"
...
const pictureSize = (imgSelector) => {
	...
	blocks.forEach(block => {
		//mouseover - когда мышь над элементом
		block.addEventListener('mouseover', () => {
			showImg(block);
		});
		//когда мышь вышла из элмента
...
const showMoreStyles = (trigger, wrapper) => {
	...
	btn.addEventListener('click', function () {
		(0,_services_requests__WEBPACK_IMPORTED_MODULE_0__.getResource)('assets/db.json')
